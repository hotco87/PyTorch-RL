...
class DDPG(object):
    ...
    def train(self):
        # (batch_size, (S,A,R,S',D)) 형태의 array를 ((S,A,R,S',D),batch_size) 형태로 변환
        ...
        # critic 신경망의 두개의 critic을 동시에 업데이트 한다.
        # actor는 critic의 업데이트가 끝난 뒤 한다.
        # critic update
        ...
        # 바로 타겟이 주어지면 actor에서만 계산이 되지만, 타겟은 액션값이 아닌 Q값으로 주어졌다.
        # 그래서 타겟 액션값과 액션값을 이용해 계산하는것이 불가능하다.
        # 이를 위해 우리가 알고있는 Q값이 얼마나 영향을 받았는지 모델링을 해서 타겟으로 사용한다.
        #  actor update
        self.actor_optimizer.zero_grad()
        pred_actions = self.actor(states)
        # 새로운 액션값을 이용해 Q값이 얼마나 바뀌었나를 보고,
        # 이를 loss값으로 사용함.
