...
def finish_episode(rewards,next_state, done):
    ...
    if done:
        ...
        advantage = (rewards[0] + args.gamma * rewards[1] + args.gamma * (rewards[2] + args.gamma * next_value)) - value
        target = rewards[0] + args.gamma * rewards[1] + args.gamma * (rewards[2] + args.gamma * next_value)
    ...
    #policy_losses.append(-(model.log_p[0] * model.log_p[1] * model.log_p[2] * advantage))
...
def main():
    ...
    for i_episode in count(1):
        ...
        if np.mean(total_reward_list[-5:], dtype=float) > env.spec.reward_threshold: # 695, line 31
            torch.save(model.state_dict(),'./SaveModel/2.actor_critic_three_step.pth')
            ...
            fig = plt.gcf()
            plt.show()
            fig.savefig('./SaveModel/2.three_step_reward_graph.png')
            plt.close()
            break
