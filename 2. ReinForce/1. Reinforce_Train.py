...
model = Policy()                                    # 객체선언, Forward Propagation, input(state) -> Model(DNN) -> output(action-policy)
...
#    >> 0.4의 확률로 0이 나오게, 0.6의 확률b로 1이 나오게끔 확률을 정함.
def select_action(state):                    # state = [ , , ,] =
    ...
    m = Categorical(probs)
    action = m.sample()
    model.saved_log_probs.append(m.log_prob(action))       # 추후 계산 편의를 위함 (log_pi(a) * Gt)
    return action.item()
...
def finish_episode():
    ...
    for r in model.rewards[::-1]:
        R = r + 0.99 * R
        rewards.insert(0, R)
    rewards = torch.tensor(rewards) #float32
    rewards = (rewards - rewards.mean()) / (rewards.std() + eps)
    for log_prob, reward in zip(model.saved_log_probs, rewards):
        policy_loss.append(-log_prob * reward)  # policy_loss (size(rewards),)
    optimizer.zero_grad()                       # 모든 weight를 0 으로 초기화
    policy_loss = torch.cat(policy_loss).sum()  # List-> (Tensor -> Cat) -> Sum ## policy_loss 안의 값은 Tensor이기 때문에 cat sum
                                                # torch.Tensor 는 append 기능이 없다. 때문에 loss function을 구할 떄 list, numpy 로
                                                # append 한 후, (torch.cat, ex) size = n ), (torch.stack, ex) size = 0)을 이용하여 sum 한다.
...
def main():
    ...
    for i_episode in count(1):  # while문 대용, cnt_episode 출력을 위함.
        ...
        for t in range(700):
            ...
            state, reward, done, _ = env.step(action) # 1step 이동 (주어진 State에서 선택한 Action을 실행)
